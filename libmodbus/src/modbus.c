...
compute_data_length_after_meta(modbus_t *ctx, uint8_t *msg, msg_type_t msg_type)
{
    ...
    if (msg_type == MSG_INDICATION) {
        ...
        case MODBUS_FC_WRITE_FILE_RECORD:
            length = (msg[ctx->backend->header_length + 7] << 8) | msg[ctx->backend->header_length + 8];
            length *= 2;
            break;
        case MODBUS_FC_WRITE_AND_READ_REGISTERS:
                 ...
                 modbus_mapping_t *mb_mapping)
{
   ...
   case MODBUS_FC_WRITE_FILE_RECORD:
      uint8_t _nb_write = req[offset + 1];
      uint16_t max_rec_len = (req[offset + 7] << 8) + req[offset + 8];
      if ( ( (_nb_write > (uint16_t)mb_mapping->nb_registers) && (_nb_write < (uint16_t)mb_mapping->start_registers) ) ||
            ( max_rec_len == 0) ) {
          rsp_length = response_exception(
              ctx,
              &sft,
              ...
              rsp,
              TRUE,
              "Illegal rec len of values (%d)  max (%d)\r\n",
              _nb_write,
          ...
          memcpy(&rsp[rsp_length], (req + offset + 2), 7);
          rsp_length += 7;
